...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--input', default=False, required=True, help="a **phylogenetic tree** representing the evolution of a gene family. The **phylogenetic tree** can be provided in standard **newick** format, or as provided by the **phylomizer pipeline** (https://github.com/Gabaldonlab/phylomizer). In that case the input should be the folder of a phylomizer output. It is expected to include a phylogenetic tree in newick format used as the input, and a rank file with the score for the different models. It will select the best available newick tree based on the score.")
    ...
    parser.add_argument('-t', '--taxonomy', help="The true distribution of species either as: a **.csv** file containing a **taxonomy** (i.e data/taxonomy.csv, included in this folder). The **taxonomy** should include at least all species contained in the **phylogenetic tree**. Or a species tree where the true phyloegenetic relationships between all the species present in the gene family tree are present. This file must end with **.newick**,**.nw** or **.nwk**.")
    parser.add_argument('-J', '--jumps', default=2, type=int, help="Minimum number of unshared taxa between one branch of the tree and its sister branch, to be considered suspicious of being en event")
    ...
    parser.add_argument('-p', '--prokaryotes', help="Name of the file containing the mnemonic codes of prokaryotic entitites. Use only if using abaccus with a phylogeny.")
...
if taxofile.endswith("csv"):
    ...
    sys.exit("Please end the taxonomy file with csv or the phylogeny with nw, nwk or newick in order for the program to run accordingly")
...
if phylogeny:
    ...
    orthotree = fn.orthogroup_tree(root_phylotree, taxo_dict, paperbag_dict, main_leaf, args.rounds, args.jumps, species_tree, spe2age)
    print("Still under development")
    print(orthotree)
else:
    taxo_dict = fn.taxonomist(taxofile)
    paperbag_dict = fn.paperbag(taxofile)
    orthotree = fn.orthogroup(root_phylotree, taxo_dict, paperbag_dict, main_leaf, args.rounds, args.jumps)
    jump = str(orthotree[2])
    cutoff = fn.abaccus(orthotree[0], orthotree[1], taxo_dict, paperbag_dict, central_sp)
